...
export const requireKeySession: RequestHandler = (request, _response, next) => {
  ...
  if (!id.success) return next(new AppError(400, "INVALID_KEY_ID", "Invalid key identifier."));
  const sealed = request.cookies[`${KEY_COOKIE_PREFIX}${id.data}`] as string | undefined;
  if (!sealed)
    return next(new AppError(401, "KEY_SESSION_EXPIRED", "Re-enter this API key to continue."));
